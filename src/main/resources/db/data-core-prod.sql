...
-- 문제 유형 설명 — 학생 문제풀이 화면 유형 안내 문구. 유형 이름은 erp_bookstore_code(gubun='T')에 있다
IF NOT EXISTS (SELECT 1 FROM erp_bookstore_qtype_desc WHERE qtype = '01')
...
-- 뱃지 마스터 (1~5) — 2026-09-14 4종 → 5종 복원. 기본 문제 결과를 다시 불합격/합격/만점 3단계로 나눈다.
-- 이미 구 4종이 들어 있는 운영 DB는 이 스크립트가 건너뛰므로 patch-erp_bookstore_badge-5types.sql로 이관한다.
